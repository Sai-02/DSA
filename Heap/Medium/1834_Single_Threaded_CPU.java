...
class Solution {
    class Pair implements Comparable<Pair> {
        ...
        Pair(int start, int time, int index) {
            this.start = start;
            this.time = time;
            this.index = index;
        }
    ...
    class Num {
        ...
        Num(int index, int start, int time) {
            this.index = index;
            this.start = start;
            this.time = time;
        }
    ...
    public int[] getOrder(int[][] tasks) {
        ...
        for (int i = 0; i < tasks.length; i++) {
            a.add(new Num(i, tasks[i][0], tasks[i][1]));
        }
        ...
        for (int i = 0; i < a.size(); i++) {
            while (time < a.get(i).start && !pq.isEmpty()) {
                Pair pair = pq.remove();
                ans[index++] = pair.index;
                time += pair.time;
            }
            time = Math.max(time, a.get(i).start);
            pq.add(new Pair(a.get(i).start, a.get(i).time, a.get(i).index));
        }
        while (!pq.isEmpty()) {
            Pair pair = pq.remove();
            ans[index++] = pair.index;
