...
class Solution {
    ...
    public int maximumRequests(int n, int[][] requests) {
        ...
        solver(0, requests, 0, degree);
        return max;
    }

    public void solver(int index, int requests[][], int count, int degree[]) {
        if (index == requests.length) {
            if (isValidated(degree)) {
                max = Math.max(count, max);
            }
            return;
        }
        solver(index + 1, requests, count, degree);
        ...
        degree[from]--;
        degree[to]++;
        solver(index + 1, requests, count + 1, degree);
        degree[from]++;
        degree[to]--;

    }

    public boolean isValidated(int degree[]) {
