...
class Solution {
    public int minDominoRotations(int[] tops, int[] bottoms) {
        boolean found1 = isSolvable(tops, bottoms, tops[0]);
        boolean found2 = isSolvable(tops, bottoms, bottoms[0]);
        if (found1 == false && found2 == false)
            return -1;
        int minSteps = Integer.MAX_VALUE;
        if (found1)
            minSteps = Math.min(minSteps, getMinRotation(tops, bottoms, tops[0]));
        if (found2)
            minSteps = Math.min(minSteps, getMinRotation(tops, bottoms, bottoms[0]));
        return minSteps;
    }

    public int getMinRotation(int tops[], int bottoms[], int val) {
        int topsFreq = 0, bottomFreq = 0, n = tops.length;
        for (int i = 0; i < n; i++) {
            if (tops[i] == val)
                topsFreq++;
            if (bottoms[i] == val)
        ...
        return Math.min(n - topsFreq, n - bottomFreq);
    }

    public boolean isSolvable(int tops[], int bottoms[], int val) {
        boolean found = true;
        for (int i = 0; i < tops.length; i++) {
            if (tops[i] == val || val == bottoms[i])
                continue;
            else
                found = false;
        }
