    ...
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String s : logs) {
            ...
            if (parts[1].equals("start")) {
                ...
                String startString[] = stString.pop().split(":");
                int id = Integer.parseInt(startString[0]);
                int result = Integer.parseInt(parts[2]) - Integer.parseInt(startString[2]) - count + 1;
                ans[id] += result;
                if (!stCount.empty()) {
                    int prevCount = stCount.pop();
                    prevCount += result + count;
                    stCount.push(prevCount);
                }
            }
        }
        return ans;
