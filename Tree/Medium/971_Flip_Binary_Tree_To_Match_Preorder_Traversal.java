...
class Solution {
    ...
    public boolean solver(TreeNode root, int[] v) {
        if (root == null)
            return true;
        if (root.val != v[i++])
            return false;
        if (root.left != null && root.left.val != v[i]) {
            ans.add(root.val);
            return solver(root.right, v) && solver(root.left, v);
        }
        return solver(root.left, v) && solver(root.right, v);
    }
}