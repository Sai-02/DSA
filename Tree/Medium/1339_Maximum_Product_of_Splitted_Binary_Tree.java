...
class Solution {
    ...
    public int solver(TreeNode root, long sum) {
        if (root == null)
            return 0;
        int left = solver(root.left, sum);
        int right = solver(root.right, sum);
        product = Math.max(product, (long) ((sum - left) * left));
        product = Math.max(product, (long) ((sum - right) * right));
