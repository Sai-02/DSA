...
class Solution {
    public boolean isScramble(String s1, String s2) {
        ...
        return solver(s1, s2, 0, 0, s1.length(), dp);
    }

    public boolean solver(String s1, String s2, int si1, int si2, int length, Boolean dp[][][]) {
        if (dp[si1][si2][length] != null)
            return dp[si1][si2][length];
        if (s1.substring(si1, si1 + length).equals(s2.substring(si2, si2 + length))) {
            dp[si1][si2][length] = true;
            return true;
        }
        for (int i = 1; i < length; i++) {
            if ((solver(s1, s2, si1, si2, i, dp) && solver(s1, s2, si1 + i, si2 + i, length - i, dp))
                    || (solver(s1, s2, si1, si2 + length - i, i, dp) && solver(s1, s2, si1 + i, si2, length - i, dp))) {
                dp[si1][si2][length] = true;
                return true;
            }
        }
        dp[si1][si2][length] = false;
        return false;
    }
}