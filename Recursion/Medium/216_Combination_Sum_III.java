...
class Solution {
    public List<List<Integer>> combinationSum3(int k, int n) {
        ...
        solver(ans, k, n, traverse, 1);
        return ans;
    }

    public void solver(List<List<Integer>> ans, int k, int n, ArrayList<Integer> traverse, int start) {
        if (k <= 0)
            return;
        for (int i = start; i <= n && i <= 9; i++) {
            if (k == 1) {
                if (n - i == 0) {
                    traverse.add(i);
                    ArrayList<Integer> list = new ArrayList<>(traverse);
                    ans.add(list);
                    traverse.remove(traverse.size() - 1);
                }
            } else {
                traverse.add(i);
                solver(ans, k - 1, n - i, traverse, i + 1);
                traverse.remove(traverse.size() - 1);
            }
        }
    }
